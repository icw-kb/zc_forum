                ...
                <div class="px-6 py-4 space-y-4">
                    @foreach($groupData['fields'] as $fieldKey => $fieldData)
                        @if($fieldData['type'] === 'boolean')
                            <label class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer transition-colors">
                                <div class="flex-1 mr-4">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $fieldData['label'] }}</span>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $fieldData['description'] }}</p>
                                </div>
                                <div class="relative">
                                    <input type="checkbox" 
                        ...
                        @elseif($fieldData['type'] === 'select')
                            <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                                <label for="{{ $groupKey }}_{{ $fieldKey }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                    {{ $fieldData['label'] }}
                                </label>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">{{ $fieldData['description'] }}</p>
                                <select wire:model="preferences.{{ $groupKey }}.{{ $fieldKey }}" 
                                        id="{{ $groupKey }}_{{ $fieldKey }}"
                                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-600 dark:text-white transition-colors shadow-sm">
                        ...
                        @elseif($fieldData['type'] === 'number' || $fieldData['type'] === 'integer')
                            <div class="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                                ...
                                <input type="number" 
                                       wire:model="preferences.{{ $groupKey }}.{{ $fieldKey }}" 
                                       id="{{ $groupKey }}_{{ $fieldKey }}"
                                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors shadow-sm"
